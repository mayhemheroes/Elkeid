...
func PostHBJoin(hb *ConnStat) {
	url := fmt.Sprintf(HBJoinUrl, getRandomManageAddr())
	resp, err := grequests.Post(url, &grequests.RequestOptions{
		JSON:           *hb,
		RequestTimeout: 5 * time.Second,
	})
	if err != nil {
		ylog.Errorf("PostHBJoin", "failed: %v", err.Error())
		return
	}
...
func PostHBEvict(hb *HeartBeatEvictModel) {
	resp, err := grequests.Post(fmt.Sprintf(HBEvictUrl, getRandomManageAddr()), &grequests.RequestOptions{
		JSON:           *hb,
		RequestTimeout: 5 * time.Second,
	})
	if err != nil {
		ylog.Errorf("PostHBEvict", "failed: %s", err.Error())
		return
	}
