...
type Connection struct {
	...
	connStatNeedSync   bool       `json:"-"` //是否需要把状态更新至leader端
}
...
func (c *Connection) GetOnlinePluginsList() []map[string]interface{} {
	c.pluginDetailLock.Lock()
	defer c.pluginDetailLock.Unlock()
	onlineTime := time.Now().Add(-120 * time.Second).Unix()
	res := make([]map[string]interface{}, 0, len(c.pluginDetail))
...
func (c *Connection) connStatWorker() {
	ylog.Infof("Connection", "%s connStatWorker start!", c.AgentID)
		...
		case <-tk.C:
			...
			if c.connStatNeedSync {
				...
				client.PostHBJoin(&client.ConnStat{
					...
					PluginsInfo: c.GetOnlinePluginsList(),
				})
			} else {
				c.connUpdateStatLock.Unlock()
			}
